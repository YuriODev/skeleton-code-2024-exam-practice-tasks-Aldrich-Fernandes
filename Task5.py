...
# Run CheckMatch with pattern for that location to make sure it is not a patter
# and also if it is a blocked cell
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            self.DisplayPuzzle()
            print("Attempts left: "+ str(self.__SymbolsLeft))
            print("Current score: " + str(self.__Score))

            removeCell = str(input("Do you want to remove a symbol? (y/n): ")).lower()
            if removeCell == "y":
                Row = -1
                Valid = False
                while not Valid:
                    try:
                        Row = int(input("Enter row number to remove: "))
                        Valid = True
                    except:
                        pass
                Column = -1
                ...
                while not Valid:
                    try:
                        Column = int(input("Enter column number to remove: "))
                        Valid = True
                    except:
                        pass

                if self.CheckforMatchWithPattern(Row, Column) == 10 or self.__GetCell(Row, Column).GetSymbol() == "@" or self.__GetCell(Row, Column).IsEmpty():
                    print("Invalid cell choosen.")
                else:
                    self.__GetCell(Row, Column).UpdateCell()
